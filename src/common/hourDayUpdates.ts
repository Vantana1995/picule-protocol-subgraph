...
export function updatePiculeDayData(event: ethereum.Event): PiculeDayData {
  let picule = PiculeFactory.load(FACTORY_ADDRESS.toHexString())!;
  let timestamp = event.block.timestamp.toI32();
  let dayID = timestamp / 86400;
  let dayStartTimestamp = dayID * 86400;
  let piculeDayData = PiculeDayData.load(dayID.toString());
  if (!piculeDayData) {
    piculeDayData = new PiculeDayData(dayID.toString());
    piculeDayData.date = dayStartTimestamp;
    piculeDayData.dailyVolumeUSD = ZERO_BD;
    piculeDayData.dailyVolumeMON = ZERO_BD;
    piculeDayData.totalVolumeUSD = ZERO_BD;
    piculeDayData.totalVolumeMON = ZERO_BD;
    piculeDayData.dailyVolumeUntracked = ZERO_BD;
  }

  piculeDayData.totalLiquidityUSD = picule.totalLiquidityUSD;
  piculeDayData.totalLiquidityMON = picule.totalLiquidityMON;
  piculeDayData.txCount = picule.txCount;
  piculeDayData.save();

  return piculeDayData as PiculeDayData;
}
...
export function updateTokenDayData(
  ...
  if (!tokenDayData) {
    ...
    tokenDayData.priceUSD = token.derivedMON.times(bundle.monPrice);
  ...
  tokenDayData.totalLiquidityMON = token.totalLiquidity.times(
    token.derivedMON as BigDecimal
  );
  tokenDayData.totalLiquidityUSD = tokenDayData.totalLiquidityMON.times(
    bundle.monPrice
  );
...
export function updateTokenHourData(
  ...
  let tokenPrice = token.derivedMON.times(bundle.monPrice);
  let isNew = false;
  if (!tokenHourData) {
...
function archiveHourData(token: Token, end: number): void {
